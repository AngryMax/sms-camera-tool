...
class Dolphin(object):
	...
	def hook_any_dolphin(self) -> bool:
		...
		if not dolphins:
			print("No available Dolphin found!")
			return False

		for dolphin in dolphins:
			# check for game instance running
			if not self.hook_onto_shared_memory('dolphin-emu.'+str(dolphin.pid)):
				print("An available Dolphin was found, but no game instance seems to be running!")
				continue
			...
			if b'opening.bnr' in self.read(0x817fffb0, 0x50) or b'opening.bnr' not in self.read(0x82FFFFb0, 0x50):
				...
				print("An available Dolphin game instance was found, but it seems the memory hasn't been expanded correctly!")
				#continue   # TODO: before release, make this REQUIRE mem extension.
		...
		print("No Dolphin game instance could be hooked onto!")
		return False
...
@gdclass
class GDInterface(Node):
	
	memory = Dolphin()
	smsoPtrs = -99	# Placeholder val
	
	def Hook(self) -> bool:
		hookSuccess = self.memory.hook_any_dolphin()
		if hookSuccess:
			print("Hooked!")
			self.initPtrs()
	...
	# Couldn't do this in __init__ for some reason
	def initPtrs(self):
		self.gpCamera = self.memory.read_u32(0x8040D0A8)
	
	
	def getCamPosX(self):
		return self.memory.read_f32(self.gpCamera + 0x10)
	
	def getCamPosY(self):
		return self.memory.read_f32(self.gpCamera + 0x14)
	
	def getCamPosZ(self):
		return self.memory.read_f32(self.gpCamera + 0x18)
	
	def getCamTargetX(self):
		return self.memory.read_f32(self.gpCamera + 0x3C)
	
	def getCamTargetY(self):
		return self.memory.read_f32(self.gpCamera + 0x40)
	
	def getCamTargetZ(self):
		return self.memory.read_f32(self.gpCamera + 0x44)
	
	
	def writeCamData(self, pos: Vector3, target: Vector3):
		self.memory.write_f32(self.gpCamera + 0x10, pos.x)
		self.memory.write_f32(self.gpCamera + 0x14, pos.y)
		self.memory.write_f32(self.gpCamera + 0x18, pos.z)
		self.memory.write_f32(self.gpCamera + 0x3C, target.x)
		self.memory.write_f32(self.gpCamera + 0x40, target.y)
		self.memory.write_f32(self.gpCamera + 0x44, target.z)
	
	
	def nopOutCameraCode(self):
		self.memory.write_u32(0x80028bf0, 0x60000000)	# Cam Pos
	
	def restoreCameraCode(self):
		self.memory.write_u32(0x80028bf0, 0xD0030000)	# Cam Pos
